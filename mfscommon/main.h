...
/* Priority poll registration: lower prio number is served first (0 = highest).
 * Use from master modules only; other binaries keep default prepend order. */
#define main_poll_register_prio(x,y,p) main_poll_register_prio_fname(x,y,p,STR(x),STR(y))
